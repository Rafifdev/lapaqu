...
<template>
  <header
    class="h-[70px] bg-white dark:bg-[#273142] border-b border-[#E2E8F0] dark:border-[#334155] px-4 md:px-8 flex items-center justify-between sticky top-0 z-20 transition-colors duration-200">
    <!-- Left: Search Box (Using reusable AppInput from Kasir Riwayat Order) -->
    <div class="flex items-center flex-1 max-w-md">
      <AppInput v-model="searchQuery" placeholder="Cari menu, pesanan, meja..." suffixIcon="search" clearable
        inputClass="!h-[42px]" />
    </div>
      ...
      <div class="relative">
        <button @click="notificationDropdownOpen = !notificationDropdownOpen"
          ...
          <span v-if="notifications.length > 0"
            class="absolute top-2 right-2 w-2.5 h-2.5 rounded-full bg-[#E11D48] border-2 border-white dark:border-[#273142]" />
        </button>
        ...
        <AppDropdownMotion placement="bottom">
          <div v-if="notificationDropdownOpen"
            ...
            <div v-else class="divide-y divide-[#E2E8F0] dark:divide-[#334155] max-h-64 overflow-y-auto">
              <div v-for="n in notifications" :key="n.id" class="p-3 hover:bg-[#F8FAFC] dark:hover:bg-[#334155]/60 cursor-pointer">
                <p class="text-sm font-bold text-[#1E293B] dark:text-white">{{ n.title }}</p>
                <p class="text-xs text-[#475569] dark:text-[#94A3B8] mt-0.5">{{ n.message }}</p>
                <span class="text-[10px] text-[#94A3B8] mt-1 block">{{ n.time }}</span>
              </div>
            </div>
          </div>
        </AppDropdownMotion>
      ...
      <div class="relative">
        <button @click="profileDropdownOpen = !profileDropdownOpen"
          ...
          <div class="hidden md:flex flex-col text-left">
            ...
            <span class="text-xs font-semibold capitalize text-[#4880FF]">
          ...
          <div v-if="profileDropdownOpen"
            ...
            <div class="px-2 py-1.5 border-b border-[#E2E8F0] dark:border-[#334155]">
              ...
              <div class="grid grid-cols-3 gap-1 mt-2">
                <button @click="switchRole('owner')" :class="[
                ...
                <button @click="switchRole('kasir')" :class="[
                  'px-2 py-1.5 rounded-lg text-xs font-bold text-center cursor-pointer transition-colors flex items-center justify-center gap-1',
                ...
                <button @click="switchRole('kitchen_staff')" :class="[
                  'px-2 py-1.5 rounded-lg text-xs font-bold text-center cursor-pointer transition-colors flex items-center justify-center gap-1',
            ...
            <button @click="router.push('/dashboard/outlets'); profileDropdownOpen = false"
            ...
            <button @click="authStore.logout(); router.push('/auth/login'); profileDropdownOpen = false"
